...
def prepare_dataset1(args, split, transform, tokenizer, text_encoder,accelerator):
    imsize = args.imsize
    if transform is not None:
        image_transform = transform
    else:
...
class SquarePad:
    def __call__(self, image):
        ...
        vp = int((max_wh - h) / 2)
        padding = (hp, vp, hp, vp)
        return transforms.functional.pad(image, padding, (255,255,255), 'constant')
...
class TextImgDataset(data.Dataset):
    ...
    def __getitem__(self, index):
        #
        if self.dataset_name.lower().find('fooddata') != -1:
            key = self.textnames[index] 
        ...
        if self.dataset_name.lower().find('recipe') != -1:
            ...
            if self.split=='train':
                img_name = '%s/images/%s' % (data_dir, textnames)
                textnames = textnames.split('.')[0]
        ...
        if self.dataset_name.lower().find('recipe') != -1:
            ...
            caps,tokens = get_caption(text_name)
            key = text_name
            _, ingre_tokens = get_caption(text_name)
            _, instruc_tokens = get_caption(text_name)
            relation = 'None'
        else:
