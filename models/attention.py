...
class LoRALinearLayer(nn.Module):
    def __init__(self, in_features, out_features, rank=16): #128
        super().__init__()
...
class TunedCrossAttention(nn.Module):
    def __init__(self, query_dim: int, 
        ...
        if self.added_kv_proj_dim is not None:
            self.add_q_proj = LoRALinearLayer(query_dim, self.inner_dim)
            self.add_k_proj = LoRALinearLayer(cross_attention_dim, self.inner_dim)
            #self.add_v_proj = LoRALinearLayer(cross_attention_dim, self.inner_dim)
